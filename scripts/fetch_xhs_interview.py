...
def main():
    if len(sys.argv) < 3:
        print("用法: python3 fetch_xhs_interview.py <feed_id> <xsec_token>")
        sys.exit(1)
    
    feed_id = sys.argv[1]
    xsec_token = sys.argv[2]
    
    print(f"正在获取帖子详情: {feed_id}", file=sys.stderr)
    
    result = fetch_feed_detail(feed_id, xsec_token)
    content = extract_interview_content(result)
    
    print(f"标题: {content['title']}")
    print(f"描述: {content['desc']}")
    ...
    for img in content['images']:
        ...
        print(f"下载第{idx}张图片...", file=sys.stderr)
